...
import (
	...
	util "github.com/gridworkz/kato/util"
...
func (a *ApplicationAction) createHelmApp(ctx context.Context, app *dbmodel.Application) error {
	labels := map[string]string{
		constants.ResourceManagedByLabel: constants.Kato,
	}
	...
	defer cancel()
	_, err = a.katoClient.KatoV1alpha1().HelmApps(helmApp.Namespace).Create(ctx2, helmApp, metav1.CreateOptions{})
	if err != nil {
...
func (a *ApplicationAction) updateHelmApp(ctx context.Context, app *dbmodel.Application, req model.UpdateAppRequest) error {
	...
	defer cancel()
	helmApp, err := a.katoClient.KatoV1alpha1().HelmApps(app.TenantID).Get(ctx, app.AppName, metav1.GetOptions{})
	if err != nil {
	...
	_, err = a.katoClient.KatoV1alpha1().HelmApps(app.TenantID).Update(ctx, helmApp, metav1.UpdateOptions{})
	return err
}
...
func (a *ApplicationAction) DeleteApp(ctx context.Context, app *dbmodel.Application) error {
	...
	return a.deleteKatoApp(app)
}

func (a *ApplicationAction) deleteKatoApp(app *dbmodel.Application) error {
	// can't delete kato app with components
...
func (a *ApplicationAction) deleteHelmApp(ctx context.Context, app *dbmodel.Application) error {
	...
	return db.GetManager().DB().Transaction(func(tx *gorm.DB) error {
		...
		if err := a.katoClient.KatoV1alpha1().HelmApps(app.TenantID).Delete(ctx, app.AppName, metav1.DeleteOptions{}); err != nil {
			if !k8sErrors.IsNotFound(err) {
				return err
			}
		}
...
func (a *ApplicationAction) Install(ctx context.Context, app *dbmodel.Application, overrides []string) error {
	...
	helmApp, err := a.katoClient.KatoV1alpha1().HelmApps(app.TenantID).Get(ctx1, app.AppName, metav1.GetOptions{})
	if err != nil {
		if k8sErrors.IsNotFound(err) {
	...
	_, err = a.katoClient.KatoV1alpha1().HelmApps(app.TenantID).Update(ctx3, helmApp, metav1.UpdateOptions{})
	if err != nil {
		return err
	}
	return errors.Wrap(err, "install app")
