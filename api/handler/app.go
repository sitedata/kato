...
func (a *AppAction) Complete(tr *model.ExportAppStruct) error {
	...
	if tr.Body.Format != "kato-app" && tr.Body.Format != "docker-compose" {
...
func (a *AppAction) ExportApp(tr *model.ExportAppStruct) error {
	// Save metadata to group directory
...
func saveMetadata(tr *model.ExportAppStruct) error {
	// Create application group directory
...
// unicode2zh Convert unicode to Chinese, and remove spaces
