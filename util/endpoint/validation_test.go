...
func TestSplitEndpointAddress(t *testing.T) {
	address := SplitEndpointAddress("http://www.google.com")
	fmt.Printf("endpoint: %s, addrss: %s, is IP? %t \n", "http://www.google.com", address, len(ValidateEndpointIP(address)) == 0)
	address = SplitEndpointAddress("http://www.baidu.com:443")
	fmt.Printf("endpoint: %s, addrss: %s, is IP? %t \n", "http://www.google.com:443", address, len(ValidateEndpointIP(address)) == 0)
