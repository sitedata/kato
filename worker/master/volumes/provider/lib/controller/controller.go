...
import (
	...
	"github.com/gridworkz/kato/worker/master/volumes/provider/lib/controller/metrics"
	"github.com/gridworkz/kato/worker/master/volumes/provider/lib/util"
)
...
func NewProvisionController(
	...
	component := "kato.io/provisioner" + "_" + id
	...
	volumeHandler := cache.ResourceEventHandlerFuncs{
		...
		DeleteFunc: func(obj interface{}) {
			...
			case "katosssc":
				...
				logrus.Infof("storage class: katosssc; path: %s; successfully delete pv hsot path", path)
			case "katoslsc":
				nodeIP := func() string {
				...
				if nodeIP == "" {
					logrus.Errorf("storage class: katoslsc; name: %s; node ip not found", pv.Name)
					return
				}
				...
				logrus.Infof("storage class: katoslsc; path: %s; successfully delete pv hsot path", path)
			default:
